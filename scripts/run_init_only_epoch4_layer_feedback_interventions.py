...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run epoch-4-targeted init-only feedback interventions on structured pruning dynamics."
    )
...
def _make_run_root(results_dir: str, dataset: str, model: str, init_seed: int) -> str:
    ...
    run_root = os.path.join(
        ...
        dataset,
        model,
        "epoch4_layer_feedback_interventions",
        f"seed_{init_seed}",
        stamp,
    )
...
def main() -> None:
    ...
    interventions = [
        {
            ...
            "actual_prune_layer_whitelist_by_epoch": None,
            "whitelist_label": "all",
        },
        {
        ...
        {
            "run_label": "prune_until_e2_then_shadow",
            "mode": "prune_only",
            "shadow_prune": False,
            ...
            "actual_prune_layer_whitelist": None,
            "actual_prune_layer_whitelist_by_epoch": None,
            "whitelist_label": "e2:all",
        },
        {
            "run_label": "prune_until_e4_then_shadow",
            "mode": "prune_only",
            "shadow_prune": False,
            "freeze_actual_updates_after_epoch": 4,
            "shadow_after_freeze": True,
            "actual_prune_layer_whitelist": None,
            "actual_prune_layer_whitelist_by_epoch": None,
            "whitelist_label": "e4:all",
        },
    ]

    for layers in ([0], [1], [2], [3], [0, 1]):
        label = _encode_layers(layers)
        interventions.append(
            {
                "run_label": f"prune_until_e4_layers{label.replace('+', '_')}_then_shadow",
                "mode": "prune_only",
                "shadow_prune": False,
                "freeze_actual_updates_after_epoch": 4,
                "shadow_after_freeze": True,
                "actual_prune_layer_whitelist": None,
                "actual_prune_layer_whitelist_by_epoch": {4: layers},
                "whitelist_label": f"e4:{label}",
            }
        )
    ...
    for intervention in interventions:
        ...
        summary = train_structured_classifier_run(
            ...
            actual_prune_layer_whitelist_by_epoch=intervention["actual_prune_layer_whitelist_by_epoch"],
